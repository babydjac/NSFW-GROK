...
class NSFWGrokDescriberPro:
    ...
    def keyword_tags(self, description):
        ...
        for keyword, (tag, weight) in tag_map.items():
            if keyword in desc:
                tags.append(f"({tag}:{weight})" if weight != 1.0 else tag)
        return tags
    ...
    def simulated_tagger_tags(self):
        return [
            "1girl", "white tank top", "thong", "shiny skin", "ass", "solo", "kitchen", "pose", "high heels"
        ]
    ...
    def process(self, image, api_key, prepend_text, append_text, score_strategy, debug_output):
        try:
            ...
            keyword_extracted = self.keyword_tags(description)
            simulated_tags = self.simulated_tagger_tags()
            style_tags = self.style_tags()

            combined_tags = score_tags + style_tags + simulated_tags + keyword_extracted
            full_prompt = ", ".join(filter(None, [prepend_text.strip(), *combined_tags, append_text.strip()]))

            if debug_output == "enable":
                full_prompt += f"\n\n[DEBUG] Keyword Tags: {keyword_extracted}\nSimulated Tagger: {simulated_tags}"
